...
function Toggle({ id, children }: ToggleProps) {
  ...
  if (!context) {
	...
	return (
		<button id={id} className={styles.toggleButton} onMouseEnter={handleMouseEnter} onClick={handleClick}>
			{children}
		</button>
	);
}
...
function List({ id, children }: ListProps) {
  ...
  const ref = useRef<HTMLMenuElement | null>(null);
	...
	return (
		<menu ref={ref} className={styles.menu} style={{ left: position?.x, top: position?.y }}>
			{children}
		</menu>
	)
}
