...
export default function CassetteTape() {
	const rotate = '0';
	return (
		<figure className={`${styles.cassetteTape} ${rotate}`}>
			...
			<figure
				...
				style={{
					clipPath: `circle(${(130 - (0 * 100) / 0) * 0.8}px at center)`
				}}
			/>
			<figure
				className={`${styles.reel} ${styles.right}`}
				style={{
					clipPath: `circle(${(30 + (0 * 100) / 0) * 0.8}px at center)`
				}}
			/>
			...
			<img draggable="false" className={styles.tapeCoverImage} src="/images/music-cover-1.png" alt="cover" />
